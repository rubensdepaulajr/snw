...
public partial class Bio_cad : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            if (Request.QueryString["ori"].ToString() == "cadprf")
                getProfissional();
            else
    ...
    private void getProfissional()
    {
        ...
        SqlParameter[] param = {
                                    dba.MakeInParam("Prf_Mtr",SqlDbType.VarChar,20,Request.QueryString["prf"])
                               };
        SqlDataReader reader;
        reader = dba.runProcDataReader("ssAtz_Prf", param);
        while (reader.Read())
        {
